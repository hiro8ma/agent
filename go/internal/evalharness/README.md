...
Gemini の無料枠は 1 分あたり 5 回・**1 日あたり 20 回**。

日次上限に当たったとき、応答の `retryDelay` は数十秒を返す。これに従って待ち直しても
回復しない。実測では 4 回再試行して 18.6 分待ち、5 回の呼び出しがすべて 429 だった。
そのため `retryAfter` は `quotaId` に `PerDay` を含む場合は待たずに諦める。

再試行が効くのは 1 分あたりの上限に当たった場合だけ。CI で常時回すなら有料枠が要る。
