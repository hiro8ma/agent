...
const baseInstruction = "あなたは天気と観光を答えるエージェントです。" +
	"聞かれたことに応じて呼ぶツールを選びます。" +
	"天気だけ聞かれたら get_weather だけ、" +
	"観光だけ聞かれたら get_sightseeing だけを呼びます。" +
	"旅行の相談や、天気と観光の両方を聞かれたときだけ両方呼びます。" +
	"聞かれていない方を呼ぶと、余分な呼び出しになります。" +
	"その結果だけを使って答えます。" +
	"両方呼んだときは組み合わせて提案します。" +
	"例えば晴れなら屋外のスポットを勧めます。" +
	"都市名が分からないときはツールを呼ばず、" +
