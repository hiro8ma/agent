// Package knowledgepgvector は pgvector を使う社内ナレッジの検索。
//
// この実装がプロセス内の検索（inmemory.Knowledge）と違うのは、検索結果の質が静かに落ちる経路を塞いでいる点にある。
...
var _ externalservice.KnowledgeSearcher = (*PgVector)(nil)
...
func (s *PgVector) Search(ctx context.Context, query string, limit int) ([]model.KnowledgeDoc, error) {
