...
| `00b_bpe_tokenizer.ts` | BPE（Byte Pair Encoding）+ 特殊トークン | 頻出ペアを反復マージ（count_pairs → merge → 反復）。`<\|endoftext\|>` を 1 ID で表す（学習は区切りで分割、encode は 1 ID に圧縮、`allowSpecial=false` で解釈無効化） |
