...
| ファイル | 概念 | 数式 |
|---|---|---|
| `00_byte_tokenizer.ts` | バイト単位トークン化 | `encode(text) = UTF-8 bytes`（vocab 0..255 固定） |
| `00b_bpe_tokenizer.ts` | BPE（Byte Pair Encoding）+ 事前トークン化 + 特殊トークン | 頻出ペアを反復マージ（count_pairs → merge → 反復）。事前トークン化は GPT-2 正規表現（`u` フラグで `\p{L}` `\p{N}` を使用、外部ライブラリ不要）で単語 / 数字 / 句読点 / 短縮形 / 空白へ分割し、マージを pretoken 内部に閉じ込める。`<\|endoftext\|>` を 1 ID で表す（学習は区切りで分割、encode は 1 ID に圧縮、`allowSpecial=false` で解釈無効化） |
...
| `14_causal_mask.ts` | Causal Mask（因果マスク） | `mask[i][j] = (j > i)`、attention の softmax 前に `-∞` を入れて未来 token を遮断 |
...
# トークン化（embedding の上流。生テキスト → token ID 列）
bun run transformer/00_byte_tokenizer.ts
bun run transformer/00b_bpe_tokenizer.ts
...
- バッチ処理（n × d_model の 2 次元のみ、batch 軸を持たない）
- Padding mask（causal mask は Step 14 で実装済み）
- Dropout
- 学習可能パラメータ（手書き定数のみ、勾配計算なし）

これらは別 commit で段階的に追加する予定。**まず内積アテンションの 4 段（生スコア → スケール → softmax → V 合成）を完璧に理解する** ことを優先。

## 実装済みのカバー範囲

- Single-Head Attention / Multi-Head Attention（Step 05 / 07）
- Encoder の完全な構造（Step 13）
- **Causal Mask 付き Self-Attention**（Step 14）→ これだけで Decoder-only（GPT/Claude）の本体構造に到達
