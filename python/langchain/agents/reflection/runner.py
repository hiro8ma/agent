"""Run helpers for the Plan→Generate→Reflect graph (streaming + final answer)."""
...
def _select_model(use_fake: bool) -> tuple[BaseChatModel, bool]:
    ...
    Falls back to the offline FakeChatModel when no LLM key is set, so the loop and
...
def stream(
    ...
    for chunk in app.stream(initial_state(task, max_iterations)):
...
def run(
    ...
    model, _ = _select_model(use_fake)
    app = build_graph(model)
    return dict(app.invoke(initial_state(task, max_iterations)))
