...
```bash
# 評価セットを作り直す
uv run python samples/agents/weather_agent/evals/build_eval_set.py

# 評価セット自体を検査する（API キー不要）
uv run pytest samples/agents/weather_agent/evals/ -q

# エージェントを評価する（API キーが要る）
adk eval samples/agents/weather_agent \
  samples/agents/weather_agent/evals/weather_agent_v1.evalset.json
```
