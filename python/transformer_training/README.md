...
| `bpe.py` | バイト単位 tokenizer + BPE（train / encode / decode / save / load）+ 特殊トークン `<\|endoftext\|>`（学習は区切りで分割、encode は 1 ID に圧縮、`allow_special=False` で解釈無効化）。char-level の代替として差し替え可能 |
