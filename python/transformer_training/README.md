...
`lora_demo.py` — LoRA の原理と、実装で壊れやすい 3 点の確認
...
`ΔW = BA` で、`B` は `d×r`、`A` は `r×d`。積は `d×d` になるがランクは `r` 以下に制限される。
「事前学習からの変更は低ランクで足りる」という仮説に基づく。

```
d=768, r=8   通常 589,824 → LoRA 12,288   97.9% 削減
d=4096, r=8  通常 16,777,216 → 65,536     99.6% 削減
```

### 壊れやすい 3 点

いずれも学習は進む。損失も下がる。動作確認では検出できない。

**B をゼロで初期化する** `ΔW = BA` なので `B = 0` なら学習開始時点で `ΔW = 0` になる。
事前学習の重みがそのまま残り、そこから少しずつ動かせる。
両方を `randn` にすると、学習前から出力が壊れる。実測した学習前のずれは次のとおり。

```
A, B とも randn   2675.1792
B をゼロ          0.0000
```

**スケールは alpha ではなく alpha / rank** そのまま掛けると、
rank を上げたときに更新の大きさまで増える。

```
alpha=16 固定で rank を変えたときの出力の大きさ
  rank    alpha そのまま   alpha / rank
     4        0.02806        0.00701
    32        0.08546        0.00267
```

`alpha / rank` なら rank を変えても更新の規模が揃うため、
rank と alpha を独立に調整できる。

**元の重みを凍結する** LoRA レイヤーを足しただけでは学習可能なパラメータは減らない。

```
凍結しない  602,880 / 602,880  100.0%
凍結する     12,288 / 602,880    2.0%
```

### GPT-2 に適用するときの注意

GPT-2 の線形層は `nn.Linear` ではなく `Conv1D` になる。

```
Conv1D  8 個（c_attn / c_proj / c_fc）
Linear  1 個（lm_head のみ）
```

`isinstance(module, nn.Linear)` で判定すると Attention にも FFN にも当たらない。
`Conv1D` は重みが転置された形で入っているため、
`in_features` / `out_features` の対応も逆になる。
