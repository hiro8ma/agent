...
教材の 3 要点を再現する:
  ...
  (5) グラマーパターン -> FSM が状態ごとに許可集合を動かし、文法違反トークンを -inf で弾く
"""
...
from grammar import FsmGrammar
...
GRAMMAR_VOCAB = [
    ...
    "Cholera",
    "1985",
    "19xx",  # year 位置で弾かれる不正トークン（英字混入）
    "NULL",
    "|",
]
GRAMMAR_ID_TO_TOK = dict(enumerate(GRAMMAR_VOCAB))
GRAMMAR_TOK_TO_ID = {t: i for i, t in GRAMMAR_ID_TO_TOK.items()}


def grammar_demo() -> None:
    print("=== (6) グラマーパターン（FSM で author | title | year を制約） ===")
    grammar = FsmGrammar(GRAMMAR_VOCAB)

    # 生のモデルが各ステップで最も出したい（しかし文法的に不正な）トークンを模す。
    # FSM の許可集合で AllowedTokens を駆動し、不正を -inf で弾いて整形済み出力に収束させる。
    target = [
    ...
    history_tok: list[str] = []
    generated: list[str] = []
    vocab_size = len(GRAMMAR_VOCAB)
    bad_id = GRAMMAR_TOK_TO_ID["19xx"]

    for want in target:
        allowed_tokens = grammar.allowed_next(state, history_tok)
        allowed_ids = {GRAMMAR_TOK_TO_ID[t] for t in allowed_tokens}

        # 生ロジット: 不正トークン '19xx' を最有力にし、本来出したい want は 2 番手に置く。
        # 制約が無ければ '19xx' が選ばれる状況を作る。
        raw = np.full(vocab_size, 0.0)
        raw[bad_id] = 5.0
        raw[GRAMMAR_TOK_TO_ID[want]] = 3.0

        def allowed_fn(_h: Sequence[int], ids: set[int] = allowed_ids) -> set[int]:
            return ids

        proc = AllowedTokensLogitsProcessor(allowed_fn)
        masked = proc(np.zeros((1, 1), dtype=int), raw.reshape(1, -1))
        probs = softmax(masked)[0]

        chosen = GRAMMAR_ID_TO_TOK[int(np.argmax(probs))]
        bad_blocked = probs[bad_id] == 0.0
        note = "" if bad_blocked else "  (!! 不正が漏れた)"
        print(f"  state={state:<7} want={want!r:<11} chosen={chosen!r:<11} '19xx' p={probs[bad_id]:.3f}{note}")

        generated.append(chosen)
        history_tok.append(chosen)
        state = grammar.step(state, chosen)

    print(f"\n  整形済み出力: {' '.join(generated)!r}")
